...
struct StelVertexArray
{
	...
	StelVertexArray(const std::vector<Vec3d>& v, StelPrimitiveType pType=StelVertexArray::Triangles,const std::vector<Vec2f>& t=std::vector<Vec2f>(), const std::vector<unsigned short> i=std::vector<unsigned short>()) :
	...
	bool isTextured() const {return !texCoords.empty();}

	bool isColored() const {return !colors.empty();}
...
Func StelVertexArray::specForeachTriangle(Func func) const
{
	switch (primitiveType)
	{
		case StelVertexArray::Triangles:
			...
			for (int i = 0; i < static_cast<int>(vertex.size()); i += 3)
			{
		...
		case StelVertexArray::TriangleFan:
		{
			...
			for (int i = 1; i < static_cast<int>(vertex.size() - 1); ++i)
			{
		...
		{
			for (int i = 2; i < static_cast<int>(vertex.size()); ++i)
			{
				if (i % 2 == 0)
