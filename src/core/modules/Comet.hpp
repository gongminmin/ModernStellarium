...
private:
	...
	void computeParabola(const float parameter, const float topradius, const float zshift, std::vector<Vec3d>& vertexArr, std::vector<float>& texCoordArr, std::vector<unsigned short>& indices, const float xOffset=0.0f);
	...
	std::vector<double> comaVertexArr;
	std::vector<float> comaTexCoordArr; //  --> 2014-08: could also be declared static, but it is filled by StelPainter...
	...
	std::vector<Vec3d> gastailVertexArr;  // computed frequently, describes parabolic shape (along z axis) of gas tail.
	std::vector<Vec3d> dusttailVertexArr; // computed frequently, describes parabolic shape (along z axis) of dust tail.
	std::vector<Vec3f> gastailColorArr;    // NEW computed for every 5 mins, modulates gas tail brightness for extinction
	std::vector<Vec3f> dusttailColorArr;   // NEW computed for every 5 mins, modulates dust tail brightness for extinction
	static std::vector<float> tailTexCoordArr;  // computed only once for all comets!
	static std::vector<unsigned short> tailIndices; // computed only once for all comets!
